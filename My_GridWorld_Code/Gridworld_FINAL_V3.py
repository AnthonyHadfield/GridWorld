...
Def state_coords
    ...
    AN array of state number and Agent coords
...
Def current_state
    ...
    takes in (1) previous state coords
    takes in (2) the move
    takes in (3) current state coords

    Puts out previous STATE
    puts out current STATE

Def Q_table

    Q_table array of state directional rewards
...
Def move_agent
    ...
    outputs (1) previous state coords (coords before move applied)
    outputs (2) the 'move'
    outputs (3) current state coords
...
class Gridworld():
    ...
    def state_coords(self):
        ...
        self.sc = np.array([[1, 70, 290], [2, 180, 290], [3, 290, 290], [4, 400, 290], [5, 70, 180],
                            [7, 290, 180], [8, 400, 180], [9, 70, 70], [10, 180, 70], [11, 290, 70], [12, 400, 70]],
                           dtype=int)
        print(self.sc)
    ...
    def current_state(self):
        ...
        self.ps_m_cs
        print('ps_m_cs', self.ps_m_cs)

        self.ps = self.ps_m_cs[0]
        self.move = self.ps_m_cs[1]
        self.cs = self.ps_m_cs[2]

        print(self.ps)
        print(self.move)
        print(self.cs)

        for x in range(len(self.sc)):
            if self.sc[x, 1] == self.ps[0] and self.sc[x, 2] == self.ps[1]:
                previous_state = self.sc[x, 0]
        print('previous state =', previous_state)

        print('move=', self.move)

        for x in range(len(self.sc)):
            if self.sc[x, 1] == self.cs[0] and self.sc[x, 2] == self.cs[1]:
                current_state = self.sc[x, 0]
    ...
    def move_agent(self):
        ...
        for j in range(1):
            ...
            for i in range(1, 2):
                ...
                previous_state = [int(s[0]), int(s[1])]
                ...
                """print as INTEGER Coords"""
                # print('move =', move)
                current_state = [int(s[0]), int(s[1])]

                self.ps_m_cs = [previous_state, move, current_state]
