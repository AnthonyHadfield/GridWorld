...
class Gridworld:
        ...
        if choice < self.epsilon:
            for i in range(11):
                if self.sr[i, 0] == previous_state:
                    ...
                    # print('state reward =', state_rewards)
                    maxq = state_rewards.index(max(state_rewards))
                    # print('maxQ =', maxQ)
                    # print('maxQ',maxQ)
                    # print('maxQ value',state_rewards[state_rewards.index(max(state_rewards))])
                    move = ['up', 'down', 'left', 'right']
                    self.move = move[maxq]
                    # print('move epsilon', self.move)
                    if state_rewards[state_rewards.index(max(state_rewards))] == 0:
                        self.move = np.random.choice(['up', 'down', 'left', 'right'])
                        # print('zero move =', self.move)
        else:
            self.move = np.random.choice(['up', 'down', 'left', 'right'])
            # print('move random =', self.move)
    ...
    def move_agent(self):
        ...
        for i in range(1, 50):
            ...
            # print('previous coords=', previous_state_coords)
